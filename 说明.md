...
将Swift Compiler Language 设置为Swift 6,报了很多错,关键是我已经不知道该如何修改了

# 20260303更新

Tuist改造的分支会和当前的develop分支打架。

如果你从Tuist分支，切到develop分支，记得要删除整个根目录下面的Tuist文件夹下面的.build文件夹，然后清理缓存之后再进行编译。

如果你从develop分支，切到Tuist分支，记得要重新tuist install，如果没有缓存这个过程可能会很长，甚至需要翻墙，成功之后再tuist tuist generate，然后清理缓存之后再进行编译
