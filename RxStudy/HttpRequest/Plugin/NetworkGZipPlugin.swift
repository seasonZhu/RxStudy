...
/// 解压缩插件，只能解开`GZip`压缩数据 对https://github.com/yangKJ/RxNetworks进行了改造 有些方法大可不必如此复杂
/// GZip plugin, Can only unpack `GZip` compressed data.
struct NetworkGZipPlugin: PluginType {
    
    /// Decompression stream size
    let GZIP_STREAM_SIZE: Int32 = Int32(MemoryLayout<z_stream>.size)
    /// Decompression buffer size
    let GZIP_BUF_LENGTH: Int = 512
    
    public func process(_ result: Result<Response, MoyaError>, target: TargetType) -> Result<Response, MoyaError> {
        if case .success(let response) = result {
            // Can only unpack GZip compressed data
            guard isGZipCompressed(response.data) else {
                return result
            }
            let data = gzipUncompress(response.data)
            let newResponse = Response(statusCode: response.statusCode,
                                     data: data,
...
extension NetworkGZipPlugin {
    ...
    public func isGZipCompressed(_ data: Data) -> Bool {
        return data.starts(with: [0x1f, 0x8b])
    }
    
    public func gzipCompress(_ data: Data) -> Data {
    ...
    public func gzipUncompress(_ data: Data) -> Data {
