...
class CoinRankListViewModel: Refreshable {
    
    var refreshStauts: BehaviorRelay<RefreshStatus> = BehaviorRelay(value: .none)
    ...
    private func refreshSuccess(model: BaseModel<Page<CoinRank>>) {
        refreshStauts.accept(.endHeaderRefresh)
        guard let curPage = model.data?.curPage, let totalPage = model.data?.total else {
            refreshStauts.accept(.hiddendFooter)
            return
        }
        
        if curPage < totalPage {
            refreshStauts.accept(.showFooter)
        }else if curPage == totalPage {
            refreshStauts.accept(.endFooterRefreshWithNoData)
        }else {
            refreshStauts.accept(.hiddendFooter)
        }
    }
    
    private func loadMoreSuccess(model: BaseModel<Page<CoinRank>>) {
        refreshStauts.accept(.endFooterRefresh)
        guard let curPage = model.data?.curPage, let totalPage = model.data?.total else {
            refreshStauts.accept(.hiddendFooter)
            return
        }
