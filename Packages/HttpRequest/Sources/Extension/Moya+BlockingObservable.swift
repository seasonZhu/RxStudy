...
/// 自己写的,对Result的扩展写法
public extension Result where Success == Moya.Response, Failure == MoyaError {
    ...
    func map<D: Decodable>(_: D.Type) -> Result<D, MoyaError> {
        switch self {
        case let .success(response):
            do {
...
public extension Result where Success == Any?, Failure == Moya.MoyaError {
    ...
    /// - Returns: Result
    func filterNil<T>(_: T.Type) -> Result<T, Moya.MoyaError> {
        switch self {
        case let .success(optional):
            switch optional {
            case .none:
