...
class RequestLoadingPlugin: PluginType {
    ...
    func willSend(_: RequestType, target: TargetType) {
        print("开始请求")
...
public class ResponseInterceptorPlugin: PluginType {
    public func process(_ result: Result<Moya.Response, MoyaError>, target _: TargetType) -> Result<Moya.Response, MoyaError> {
        switch result {
        case let .success(response):
...
public extension ResponseInterceptorPlugin {
    /// 错误的服务端业务编码
